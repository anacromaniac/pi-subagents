...
**Subagents start state** (`subagentsEnabled`, default `false`): what a new session starts as. Off, a fresh session loads with the `Agent`, `get_subagent_result` and `steer_subagent` tools (and `SubagentWorkflow`, when it is on) withdrawn from the active set — the model is never told they exist and their specs cost no system-prompt tokens — and `@handle` at the prompt is ordinary prose. `/agents on` brings them back for the current session and `/agents off` takes them away, aborting every running agent and workflow and stopping the scheduler; the footer reads `Subagents: on` / `Subagents: off`. The toggle is **session-scoped and never written back to `subagents.json`**, so the next session starts disabled again — set this field to `true` to start enabled. Read at `session_start`, so a change applies on the next session. This is deliberately not a `/agents → Settings` entry: that menu persists, and persisting the toggle is exactly what the default is avoiding. A one-line note in the system prompt states the current state (`Subagents are OFF for this session. The user toggles them with /agents on.`), so the model knows the feature exists and answers "can you use subagents?" directly rather than inferring it from tools that are absent on purpose.
